...
class OverlayView(context: Context?, attrs: AttributeSet?) :
    ...
    private var selectedPoint = mutableListOf<Pair<Float, Float>>()
//    private var overlayColor: String = "#8012B5CB"
    private var overlayColor: String = "#FFFFFF"
    ...
    private val selectPaint = Paint().apply {
        color = selectionMarkerColor.toColorInt()
    }
    private val borderPaint = Paint().apply {
        color = selectionMarkerBorderColor.toColorInt()
    }
    ...
    fun setMaskResult(byteBuffer: ByteBuffer, maskWidth: Int, maskHeight: Int) {
        ...
        maskBitmap = bitmap.scale(scaleWidth, scaleHeight, false)
        invalidate()
    }

    fun setSelectPosition(x: Float, y: Float) {
        selectedPoint.clear()
        selectedPoint.add(Pair(x,y))
        invalidate()
    }
    ...
    fun addSelectPosition(x: Float, y: Float) {
        selectedPoint.add(Pair(x,y))
        invalidate()
    }
}