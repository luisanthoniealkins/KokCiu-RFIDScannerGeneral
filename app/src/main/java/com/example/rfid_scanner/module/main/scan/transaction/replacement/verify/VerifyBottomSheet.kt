...
class VerifyBottomSheet(private val listener: VerifyListener, private val verifyTags: List<Tag>) :
    ...
    private fun setUpViews() = with(binding){
        ...
        btnConfirm.setOnClickListener {
            if (!viewModel.verifyTagAdapter.isAllVerified()) {
                showToast("Belum semua tag terverifikasi")
                return@setOnClickListener
            }

            val errorSimilar = viewModel.checkSimilarTags()
            if (errorSimilar.isNotEmpty()) {
                showToast("Potensi kode duplikat")
                return@setOnClickListener
            }
